...
Drop `DeepDreamLive.tox` into a network, install the Python environment once,
and turn **Active** on.

## What you need

- TouchDesigner. Current builds use Python 3.11. Confirm it in the Textport:

```python
sys.version
```

- The same Python version installed on the machine (`py -3.11 --version`).
...
3. Copy the PowerShell command from Status (or the Textport) and run it.
   For Python 3.11 and CUDA it looks like this:

```powershell
powershell -ExecutionPolicy Bypass -File "C:\path\to\TouchDeepDream\install_dependencies.ps1" -Device cuda -PythonVersion 3.11
```

4. When it finishes, pulse **Check Dependencies**. A CUDA setup reports the
   PyTorch version and `CUDA=True`. The check runs in `venv_td`, outside
   TouchDesigner.
